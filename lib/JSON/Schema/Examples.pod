...
=head2 Validate some JSON contact info against nested schemas.
 ...
 my $card_schema = from_json(get('http://json-schema.org/card'));
 
 # Hmmm... but this schema references a couple of other
 # schema for addresses and geographic co-ordinates. Let's
 # expand those references...
 my $hyper = JSON::Hyper->new;
 my $full_card_schema = $hyper->process_includes($card_schema);
 
 # Now we create our JSON Schema validator:
 my $validator = JSON::Schema->new($full_card_schema);
